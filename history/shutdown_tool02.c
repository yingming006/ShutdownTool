...
void ScheduleShutdown(int days, int hours, int minutes, int operation) {
    int totalSeconds = days * 86400 + hours * 3600 + minutes * 60;
    if (totalSeconds <= 0) {
        MessageBoxW(NULL, L"请输入一个大于0的时间", L"错误", MB_ICONERROR);
        return;
    }

    wchar_t command[50];
    wchar_t operationName[20];
    if (operation == ID_SHUTDOWN) {
        swprintf(command, 50, L"shutdown -s -t %d", totalSeconds);
        wcscpy(operationName, L"关机");
    } else if (operation == ID_RESTART) {
        swprintf(command, 50, L"shutdown -r -t %d", totalSeconds);
        wcscpy(operationName, L"重启");
    } else if (operation == ID_SLEEP) {
        swprintf(command, 50, L"shutdown -h -t %d", totalSeconds);
        wcscpy(operationName, L"睡眠");
    }

    if (RunProcess(command)) {
        wchar_t message[150];
        swprintf(message, 150, L"计算机将在 %02d 天 %02d 时 %02d 分后执行 %s 操作", days, hours, minutes, operationName);
        MessageBoxW(NULL, message, L"成功", MB_ICONINFORMATION);
    } else {
        MessageBoxW(NULL, L"定时操作失败，请检查权限或系统设置", L"错误", MB_ICONERROR);
    }
}
...
void CancelShutdownTool() {
    if (RunProcess(L"shutdown -a")) {
        MessageBoxW(NULL, L"已取消定时操作", L"成功", MB_ICONINFORMATION);
    } else {
        MessageBoxW(NULL, L"取消操作失败，请检查权限或系统设置", L"错误", MB_ICONERROR);
    }
}
